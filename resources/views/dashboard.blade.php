    ...
    <div class="p-6">
        @auth
            @if(auth()->user()->role === 'admin')
                <div class="alert alert-warning mb-4">現在：<b>管理者</b></div>
            ...
            @elseif(auth()->user()->role === 'guest')
                <div class="alert alert-info mb-4">現在：<b>ゲスト様</b></div>
                <a href="{{ route('community.index') }}" class="btn btn-primary ml-2">コミュニティへ</a>

            @elseif(auth()->user()->subscribed('default'))
                <div class="alert alert-success mb-4">現在：<b>有料会員様</b></div>
                <a href="{{ route('community.index') }}" class="btn btn-primary ml-2">コミュニティへ</a>
                <a href="{{ route('billing.portal') }}" class="btn btn-outline">支払い情報の確認・変更</a>

            @else
                <div class="alert alert-info mb-4">現在：<b>無料会員様</b></div>
                <a href="{{ route('billing.show') }}" class="btn btn-primary">有料会員お申し込み</a>
            @endif
        @endauth
    </div>
</x-app-layout>
